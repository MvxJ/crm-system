...
const CustomerMessagesTab = ({ customerId }) => {
  ...
  const fetchData = async (items) => {
    ...
    try {
      const response = await instance.get(`/messages/list?customerId=${customerId}&items=${items}&order=DESC&orderBy=createdDate`);
      ...
      if (response.status !== 200) {
        ...
        const newData = response.data.results.messages;
