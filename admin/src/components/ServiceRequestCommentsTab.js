...
import { Col, Popconfirm, Row, Spin, Tooltip, notification } from "../../node_modules/antd/es/index";
...
const ServiceRequestCommentsTab = ({ serviceRequestId }) => {
    ...
    const fetchData = async (items) => {
        ...
        try {
            const response = await instance.get(`/comment/${serviceRequestId}/list?items=${items}&order=ASC&orderBy=createdDate`);
            ...
            setMaxResults(
                response.data.data ? response.data.data.maxResults : 0
            );
            setData(newData);
        } catch (error) {
            notification.error({
                    ...
                    <Row>
                        <Col span={24}>
                            {data.length > 0 ? data.map((comment) => (
                                (comment.isHidden === true && currentUser.id === comment.author.id) || comment.isHidden == false ?
                                    <Row key={comment.id}>
                                        <Col span={24} style={{textAlign: 'right', marginBottom: '5px'}}>
                                            {currentUser && currentUser.id === comment.author.id && (
                                                <div>
                                                    <Tooltip title={comment.isHidden === true ? 'Show Comment' : 'Hide Comment'}>
                                                        <Button
                                                            type="primary"
                                                            onClick={() => handleToggleHideComment(comment.id, !comment.isHidden)}
                                                        >
                                                            {comment.isHidden === true ? <EyeOutlined /> : <EyeInvisibleOutlined />} {comment.isHidden === true ? 'Show Comment' : 'Hide Comment'}
                                                        </Button>
                                                    </Tooltip>
                                        ...
                                        <Col span={24}>
                                            <Row>
                                                <Col span={24}>
                                                    <Link to={`/administration/users/detail/${comment.author.id}`}>
                                                        <UserOutlined /> {comment.author.email}
                                                    </Link>
                                                </Col>
                                            </Row>
                                            <div style={{ marginBottom: '16px', border: '1px solid #ddd', padding: '8px' }}>
                                                <div>
                                                    <b>Author: </b>
                                                    <span style={{ marginLeft: '10px' }}>{comment.createdAt ? FormatUtils.formatDateWithTime(comment.createdAt.date) : '-'}</span>
                                                </div>
